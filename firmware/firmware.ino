...
// Ersatz für Arduino map() (vermeidet seltene Konflikte)
...
void setup() {
  ...
  tft.setRotation(3);                  // 3 = Landscape, Uhr oben rechts
  ...
  server.on(
    ...
    [](AsyncWebServerRequest *r, String filename, size_t index, uint8_t *data, size_t len, bool final) {
      if (!index) {
        // Optional: Content-Type prüfen
        Update.onProgress([](size_t, size_t) {});
