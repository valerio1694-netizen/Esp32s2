...
volatile bool softStop = false;     // STOP gedrückt
volatile bool faultLatched = false; // Störung muss quittiert werden
...
void applyCabinet(){
  if(!cabOn){ fill_solid(cabLeds, NUM_CAB, CRGB::Black); return; }
  CRGB w = CRGB::White;
...
void applyTower(){
  ...
  if(state == STATE_HOME) towerLeds[0] = CRGB(0,0,255);
  if(state == STATE_RUN)  towerLeds[1] = CRGB(0,255,0);
  if(state == STATE_WAIT) towerLeds[2] = CRGB(255,180,0);
  if(state == STATE_ERR)  towerLeds[3] = CRGB(255,0,0);
}

void renderAll(){
  applyTower();
  applyCabinet();
  FastLED.setBrightness(towerBrightness);   // global (Cabinet ist schon skaliert)
  FastLED.show();
}
...
// ===== "Panel Taster" Logik =====
void pressStop(){
  // Soft-Stop: keine Störung
  softStop = true;
  if(!faultLatched) setState(STATE_WAIT);
}

void pressStart(){
  // Start nur wenn Mode != 0 und keine latched Störung
...
void pressReset(){
  // Reset quittiert nur latched Fault
  if(faultLatched){
    faultLatched = false;
    softStop = false;
    setState(STATE_HOME);
  } else {
    // optional: auch SoftStop aufheben
    softStop = false;
    if(state == STATE_WAIT) setState(STATE_HOME);
  }
}
